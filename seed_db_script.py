...
N_TG_USERS     = int(os.getenv("SEED_N_TG_USERS", "80"))
...
def tx_details(cat: str):
    if cat == "transfer": return None, None
    receiver, items = random.choice(RECEIVER_CATALOG[cat]); return receiver, random.choice(items)
...
async def truncate_all(conn):
    # child -> parent order; CASCADE for safety
